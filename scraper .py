...
# Try multiple Workday URL patterns — they vary by tenant
WORKDAY_URL_PATTERNS = [
    "https://{tenant}.wd5.myworkdayjobs.com/wday/cxs/{tenant}/External_Career_Site/jobs",
    "https://{tenant}.wd1.myworkdayjobs.com/wday/cxs/{tenant}/External_Career_Site/jobs",
    "https://{tenant}.wd3.myworkdayjobs.com/wday/cxs/{tenant}/External_Career_Site/jobs",
    "https://{tenant}.wd10.myworkdayjobs.com/wday/cxs/{tenant}/External_Career_Site/jobs",
]
...
async def scrape_workday(session: aiohttp.ClientSession, system: str, tenant: str) -> list[Job]:
    ...
    working_url = None
    for pattern in WORKDAY_URL_PATTERNS:
        url = pattern.format(tenant=tenant)
        try:
            async with session.post(url,
                json={"limit": 20, "offset": 0, "searchText": "", "locations": [], "categories": []},
                headers={**HEADERS, "Content-Type": "application/json"},
                proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=20)) as r:
                if r.status == 200:
                    working_url = url
                    break
        except:
    ...
    if not working_url:
        logger.debug(f"Workday {system}: no working URL found for tenant '{tenant}'")
        return []
...
async def scrape_taleo(session: aiohttp.ClientSession, system: str, org: str) -> list[Job]:
    ...
    try:
        ...
        async with session.get(
            ...
            proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)) as r:
            if r.status != 200:
                logger.debug(f"Taleo {system}: HTTP {r.status}")
                return []
    ...
    while True:
        try:
            ...
            logger.debug(f"Taleo {system} page {page}: {e}")
            break
    return jobs
    ...
    while True:
        try:
            async with session.get(
                ...
                proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)) as r:
                if r.status != 200:
                    logger.debug(f"iCIMS {system}: HTTP {r.status}")
                    break
                text = await r.text()
    ...
    while True:
        try:
            async with session.get(url,
                ...
                proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)) as r:
                if r.status != 200:
                    logger.debug(f"SuccessFactors {system}: HTTP {r.status} at {url}")
                    break
                data = await r.json(content_type=None)
...
async def scrape_greenhouse(session: aiohttp.ClientSession, system: str, org: str) -> list[Job]:
    try:
        async with session.get(
            ...
            proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)) as r:
            if r.status != 200:
                logger.debug(f"Greenhouse {system}: HTTP {r.status}")
                return []
            data = await r.json()
        jobs = []
    ...
    while True:
        try:
            async with session.get(
                ...
                proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)) as r:
                if r.status != 200:
                    logger.debug(f"SmartRecruiters {system}: HTTP {r.status}")
                    break
                data = await r.json()
...
async def scrape_lever(session: aiohttp.ClientSession, system: str, org: str) -> list[Job]:
    try:
        async with session.get(
            ...
            proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)) as r:
            if r.status != 200:
                logger.debug(f"Lever {system}: HTTP {r.status}")
                return []
    ...
    for system_name, org_code in ORGS:
        try:
            async with session.get(
                ...
                if r.status != 200:
                    logger.debug(f"USAJOBS {system_name}: HTTP {r.status}")
                    continue
                data = await r.json()
