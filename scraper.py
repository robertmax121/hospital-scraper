...
_RQ_WORD = (r"(?:minimum|basic|required|preferred|desired|additional|job|position|typical|special|other|general|"
            ...
            r"qualifications?|requirements?|education(?:al)?|experience|licens(?:e|es|ure|ures|ing)|certifications?|"
            r"registrations?|credentials?|knowledge|skills?|abilities|expectations|ksas?|training|professional|clinical|work|"
...
_RQ_LIC_RX = re.compile(
    ...
    r"|\b(?:in|by) (?:the )?state of (?:" + _US_STATES + r")\b(?=[^.;\n]{0,40}\b(?:certif|licens|registr|eligible))", re.I)
    ...
    for raw in t.split("\n"):
        ...
        for piece in ([s] if len(s) <= 300 else _rq_clauses(s)):
            ...
            if kind == "edu":
                ...
                # "EDUCATION: ... Current appropriate State licensure."
                for c in _rq_clauses(piece):
                    if _rq_lic(c):
                        add("licensure", c, _rq_pref(c, mode) if (_RQ_PREF_RX.search(c) or _RQ_REQ_RX.search(c)) else pref)
                continue
            for c in _rq_clauses(piece):
                cp = _rq_pref(c, mode) if (_RQ_PREF_RX.search(c) or _RQ_REQ_RX.search(c)) else pref
                types = _rq_types(c)
                if kind == "lic" and force and not types & {"licensure", "certifications"} and _rq_credential_line(c):
                    types.add("licensure")
                elif kind in ("lic", "lic+cert") and force and "licensure" not in types \
                        and _RQ_LIC_LINE_RX.search(c) and _rq_credential_line(c, 300):
                    types.add("licensure")
