...
WORKDAY_TENANTS = {
    ...
    # Baylor Scott & White — Phenom People platform, scraped via scrape_phenom_dom()
...
TALEO_ORGS = {
    ...
    "Surgery Center (USPI)":   "uspi",
    "Kindred Healthcare":      "kindredhealthcare",
    "Genesis Healthcare":      "genesishcc",
...
ICIMS_ORGS = {
    ...
    # Ascension Health — Phenom People platform, scraped via scrape_phenom_dom()
...
async def scrape_infor(session: aiohttp.ClientSession, system: str, org_data: tuple) -> list[Job]:
    ...
    try:
        async with req(session, "get", 
            ...
            headers={**HEADERS, "Accept": "application/json"}, ssl=False, proxy=proxies.get(), timeout=aiohttp.ClientTimeout(total=25)
        ) as r:
            if r.status != 200:
...
# ══════════════════════════════════════════════════════════════════════════
#  PHENOM DOM SCRAPER  (BSW / HCA / Ascension — all Phenom People platform)
#  Reads rendered job cards directly from the DOM rather than intercepting
#  the /widgets API (whose response key varies per site and per page state).
# ══════════════════════════════════════════════════════════════════════════
...
async def scrape_phenom_dom(browser, system_name: str, base_url: str) -> list:
    ...
    Navigates paginated search results and extracts job cards directly from HTML.
    Works regardless of which /widgets API key Phenom uses for that tenant.
    """
    jobs = []
    domain = base_url.split("/")[2]   # e.g. "jobs.bswhealth.com"
        ...
        while page_num < max_pages:
            ...
            for anchor in anchors:
                try:
                    ...
                    # Phenom job URLs: /us/en/job/{numeric_id}/slug
                    ...
                    # Title — try attribute first (most reliable), then child elements
            ...
            logger.info(
                f"  [{system_name}] page {page_num+1}: {page_jobs} new jobs "
...
async def run_playwright_scrapers() -> list[Job]:
    ...
    CUSTOM_SITES = [
        ...
        # BSW, HCA, Ascension moved to scrape_phenom_dom() above
