    ...
    while True:
        try:
            async with session.get(
                ...
                proxy=proxies.get(), ssl=False, timeout=aiohttp.ClientTimeout(total=30)) as r:
                if r.status != 200:
...
# ══════════════════════════════════════════════════════════════════════════
# SuccessFactors real public API: https://{slug}.jobs.com/api/apply/v2/jobs
# slug = the company's subdomain on their SAP-hosted career portal
# Session passed in should be direct (no ssl=False) — SF rejects proxy SSL
SUCCESSFACTORS_ORGS = {
    ...
    "Emory Healthcare":     "emory",
    "Hartford HealthCare":  "hartfordhealthcare",
}

async def scrape_successfactors(session: aiohttp.ClientSession, system: str, slug: str) -> list[Job]:
    jobs, start = [], 0
    url = f"https://{slug}.jobs.com/api/apply/v2/jobs"
    while True:
        try:
            async with session.get(url,
                params={"domain": f"{slug}.jobs.com", "start": start, "num": 25},
                headers={**HEADERS, "Referer": f"https://{slug}.jobs.com/"},
                timeout=aiohttp.ClientTimeout(total=30)) as r:
                if r.status != 200:
                    logger.info(f"SuccessFactors {system}: HTTP {r.status}")
                    break
                data = await r.json(content_type=None)
...
GREENHOUSE_ORGS = {
    ...
    "LifeStance Health":    "lifestancehealth",
    "Headway":              "headwayhealth",
    "Brightspring":         "brightspringhealthservices",
    # Removed (404): carbonhealth, includedhealth, osmind, brightspring, aveanna
}
...
async def scrape_phenom(session: aiohttp.ClientSession, system: str, base_url: str) -> list[Job]:
    ...
    # Different Phenom tenants use different paths
    endpoints = [
        f"{base_url}/api/jobs",
        ...
        f"{base_url}/api/apply/v2/jobs",  # some Phenom tenants use this
        f"{base_url}/en/jobs",
    ...
    for ep in endpoints:
        for method in ("GET", "POST"):
            try:
                ...
                if method == "GET":
                    kwargs["params"] = {"start": 0, "num": 1, "from": 0, "size": 1}
                    resp_cm = session.get(ep, **kwargs)
                else:
                    kwargs["json"] = {"query": "", "size": 1, "from": 0, "filters": [], "facets": []}
                    resp_cm = session.post(ep, **kwargs)
                ...
                async with resp_cm as r:
                    if r.status == 200:
                        ...
                        if "json" in ct:
                            ...
                            # Make sure it has jobs-like data
                            has_data = (
                                body and isinstance(body, dict) and
                                (body.get("jobs") or body.get("results") or
                                 body.get("requisitions") or body.get("total") is not None or
                                 (body.get("data") and isinstance(body.get("data"), dict)))
                            )
                            if has_data:
                                api_url = ep
    ...
    logger.info(f"Phenom {system}: using {'POST' if use_post else 'GET'} {api_url}")
    offset = 0
    while True:
        try:
            if use_post:
                resp_cm = session.post(
                    api_url,
                    json={"query": "", "size": 20, "from": offset, "filters": [], "facets": []},
            ...
            listings = (
                ...
                data.get("results") or
                (nested.get("jobs") if isinstance(nested, dict) else None) or
                []
            )
            if not listings:
                break
    ...
    while True:
        try:
            async with session.get(
                ...
                proxy=proxies.get(), ssl=False, timeout=aiohttp.ClientTimeout(total=30)
            ) as r:
                if r.status != 200:
...
async def scrape_infor(session: aiohttp.ClientSession, system: str, org_data: tuple) -> list[Job]:
    ...
    # Infor CloudSuite HCM job board backing API — try multiple known paths
    json_api = f"https://{org_id}.inforcloudsuite.com/hcm/Jobs/page/JobsSearchPage"
    try:
        async with session.get(
            json_api,
            params={
...
async def run_all() -> list[dict]:
    ...
    async with aiohttp.ClientSession(connector=proxy_connector, headers=HEADERS) as proxy_session, \
        ...
        ats_results = await asyncio.gather(
            ...
            run_taleo(direct_session),       # direct — ssl=False breaks taleo.net
            run_icims(proxy_session),
            run_successfactors(direct_session), # direct — ssl=False breaks SF
            run_greenhouse(proxy_session),
            run_smartrecruiters(proxy_session),
            run_lever(proxy_session),
            run_usajobs(direct_session),     # direct — federal gov
